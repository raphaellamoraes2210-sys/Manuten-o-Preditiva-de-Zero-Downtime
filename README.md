...
* A aluna **Raphaella Moraes** investigou estatisticamente a variável **process_temperature_k** (temperatura de processo) como possível preditor de falhas. O teste de hipótese foi aplicado com o objetivo de verificar se existe diferença significativa nos valores de temperatura entre máquinas com falha e máquinas sem falha. Inicialmente, foi realizado o teste de normalidade de Shapiro-Wilk. Os resultados indicaram que os dados **[seguem/não seguem] distribuição normal (p = X)**, o que levou à escolha do teste estatístico adequado. Dessa forma, foi aplicado o teste de Mann-Whitney, que apresentou **p-valor = X**, indicando que **[não há evidência suficiente / há evidência suficiente] para rejeitar a hipótese nula**. Para complementar a análise, também foi considerado o resultado do Teste t, que apresentou comportamento consistente com o teste não paramétrico. O tamanho do efeito foi calculado por meio do d de Cohen, resultando em **d = X**, indicando um efeito de magnitude **[baixa/média/alta]**. Assim, conclui-se que a variável temperatura de processo **[não apresenta / apresenta] diferença estatisticamente significativa entre máquinas com falha e sem falha neste dataset**. Do ponto de vista da modelagem preditiva, isso sugere que a temperatura de processo **[não é um forte preditor isolado / pode ser considerada um preditor relevante]** de falhas. Entretanto, a variável pode contribuir para o desempenho do modelo de Machine Learning quando combinada com outras variáveis do sistema, como torque, vibração e tempo de operação.
